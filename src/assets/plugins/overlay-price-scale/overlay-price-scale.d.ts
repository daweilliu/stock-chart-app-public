...
export interface RendererData {
	...
	coordinateToPrice: (coordinate: number) => BarPrice | null;
	priceToCoordinate: (price: number) => Coordinate | null;
	options: OverlayPriceScaleOptions$1;
}
export interface Label {
	label: string;
	y: number;
}
declare class OverlayPriceScaleRenderer implements IPrimitivePaneRenderer {
	_data: RendererData | null;
	update(data: RendererData): void;
	draw(target: CanvasRenderingTarget2D): void;
	_calculatePriceScale(height: number, data: RendererData): Label[];
}
declare class OverlayPriceScaleView implements IPrimitivePaneView {
	_renderer: OverlayPriceScaleRenderer;
	constructor();
	renderer(): IPrimitivePaneRenderer;
	update(data: RendererData): void;
}
interface OverlayPriceScaleOptions$1 {
	textColor: string;
...
export declare class OverlayPriceScale implements ISeriesPrimitive<Time> {
	...
	_requestUpdate?: () => void;
	_options: OverlayPriceScaleOptions$1;
	constructor(options: Partial<OverlayPriceScaleOptions$1>);
	applyOptions(options: Partial<OverlayPriceScaleOptions$1>): void;
...
export {
	OverlayPriceScaleOptions$1 as OverlayPriceScaleOptions,
};

...
var c = (s, t, e) => y(s, typeof t != "symbol" ? t + "" : t, e);
...
function R(s) {
  if (B(s)) return s * 1e3;
  if (F(s)) return new Date(s.year, s.month, s.day).valueOf();
...
function V(s, t, e = 1, i) {
  const o = Math.round(t * s), n = Math.round(e * t), a = Y(n);
...
function P(s, t, e, i) {
  const o = s.tooltips[t].lineContent, n = M(
    e,
    o,
  ...
  _showTooltip(t) {
    ...
    for (let u = 0; u < Math.min(2, t.points.length); u++) {
      ...
      if (g) {
        ...
        l.push([m, p.index]);
        const f = e.priceToCoordinate(m) ?? -1e3, [x, E] = k(
